...
DEV_BALLERINA_PROJECT_BUILD_JAR=$(realpath ".")/target/bin/${DEV_BALLERINA_MODULE_NAME}.jar

if [ ! -f "${DEV_BALLERINA_PROJECT_BUILD_JAR}" ]; then
  echo "Ballerina build Jar: ${DEV_BALLERINA_PROJECT_BUILD_JAR} not found"
  bash "${DEV_BALLERINA_ROOT_DIR}/scripts/ballerina-project/build.sh" "${DEV_BALLERINA_PROJECT_NAME}" "${DEV_BALLERINA_MODULE_NAME}"
fi
...
echo "Running Ballerina Module ${DEV_BALLERINA_MODULE_NAME} in Project ${DEV_BALLERINA_PROJECT_NAME}"
echo
