...
def clone_stdlibs(no_cache: bool, output_dir: str, name_overrides_file: str, cleanup: bool):
    repos_list = _build_stdlibs_order(no_cache, name_overrides_file)
...
@click.option("--name-overrides-file", type=str, default="std-lib-name-overrides.properties")
def execute_command(command: Tuple[str], no_cache: bool, stdlibs_dir: str, name_overrides_file: str):
    repos_list = _build_stdlibs_order(no_cache, name_overrides_file)
...
def _build_stdlibs_order(no_cache: bool, name_overrides_file: str) -> List[stdlibs.Repo]:
    if not no_cache and cache.contains(_STDLIBS_REPO_LIST_CACHE_KEY):
        ...
        repos_list: List[stdlibs.Repo] = stdlibs.get_ordered_std_lib_repos(std_lib_name_overrides)
